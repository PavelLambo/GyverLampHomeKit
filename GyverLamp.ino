...
#define EEPROM_START_ADDRESS 1408
boolean Home_ONflag = true;
//-home
...
void builtinledSetStatus(bool on) {
	digitalWrite(BI_LED_PIN, on ? LOW : HIGH);
}
...
void setup() {
  ...
  EEPROM.begin(1408+202+50);
  delay(50);
  if (EEPROM.read(EEPROM_START_ADDRESS+198) != 20) {   // первый запуск
    EEPROM.write(EEPROM_START_ADDRESS+198, 20);
    EEPROM.commit();

    for (byte i = 0; i < MODE_AMOUNT; i++) {
      EEPROM.put(EEPROM_START_ADDRESS+(3 * i + 40), modes[i]);
      EEPROM.commit();
    }
    for (byte i = 0; i < 7; i++) {
      EEPROM.write(EEPROM_START_ADDRESS+(5 * i), alarm[i].state);   // рассвет
      eeWriteInt(5 * i + 1, alarm[i].time);
      EEPROM.commit();
    }
    EEPROM.write(EEPROM_START_ADDRESS+199, 0);   // рассвет
    EEPROM.write(EEPROM_START_ADDRESS+200, 0);   // режим
    EEPROM.commit();
  }
  for (byte i = 0; i < MODE_AMOUNT; i++) {
    EEPROM.get(EEPROM_START_ADDRESS+(3 * i + 40), modes[i]);
  }
  for (byte i = 0; i < 7; i++) {
    alarm[i].state = EEPROM.read(EEPROM_START_ADDRESS+(5 * i));
    alarm[i].time = eeGetInt(5 * i + 1);
  }
  dawnMode = EEPROM.read(EEPROM_START_ADDRESS+199);
  currentMode = (int8_t)EEPROM.read(EEPROM_START_ADDRESS+200);
  // dawnMode = EEPROM.read(199);
  // currentMode = 0;//(int8_t)EEPROM.read(200);
  Serial.println("EEP done");
...
void eeWriteInt(int pos, int val) {
  int address = EEPROM_START_ADDRESS + pos;
  byte* p = (byte*)&val;
...
void homekit_setup() {
  ...
  Serial.println("acc init done");
	...
	WiFi.macAddress(mac);
	int name_len = snprintf(NULL, 0, "%s_%02X%02X%02X22",
			name.value.string_value, mac[3], mac[4], mac[5]);
	char *name_value = (char*) malloc(name_len + 1);
	snprintf(name_value, name_len + 1, "%s_%02X%02X%02X22",
			name.value.string_value, mac[3], mac[4], mac[5]);
	name.value = HOMEKIT_STRING_CPP(name_value);

  Serial.println("14 done");
	arduino_homekit_setup(&config);
  Serial.println("arduino_homekit_setup done");
  //blink_led();
  Serial.println("homekit_setup done");
}