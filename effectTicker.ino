...
void changePower() {
  Serial.println("changePower start");
  if (ONflag) {
    effectsTick();
    ...
    // delay(2);
    // FastLED.show();
    FastLED.clear(); // Очистка состояния всех светодиодов
    FastLED.setBrightness(0); // Явно устанавливаем яркость в 0 для уверенности
    FastLED.show(); // Применяем изменения
  }
  Serial.println("changePower finish");
}